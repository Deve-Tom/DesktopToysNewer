...
CGame::CGame(HWND hWnd, float x, float y, float w, float h):m_hWnd(hWnd),m_x(x),m_y(y),m_width(w),m_height(h){
	//保存全局指针
	g_game = this;
...
void CGame::Draw(){
	...
	//根据游戏当前阶段，画不同的东西
	{
		switch (m_eStatus) {
		case CGame::EGameStatusSelect:
			//画菜单暂时注释掉,因为后面的代码还没实现
			m_menu->Draw(gh);
			break;
		case CGame::EGameStatusNormal:
			//画出工具暂时注释掉,因为后面的代码还没实现
			m_pTool->Draw(gh);
			break;
		default:
			break;
...
void CGame::DrawFps(Gdiplus::Graphics & gh)
{
	...
	if (GetTickCount() - dwLast >= 1000) {//如果超过1秒钟，则输出fps,并清零计数器
		fps = m_fps;
		m_fps = 0;
		dwLast = GetTickCount();
	}
